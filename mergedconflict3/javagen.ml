...
and gen_stmt = function 
  ...
  | For_t(expr1, expr2, expr3, block) -> "for ( int " ^ gen_expr expr1 ^ "=" ^ gen_expr expr2 ^ "; <=" ^ gen_expr expr3 ^ ";" ^ gen_expr expr1 ^ "++)"^ gen_block block
  | While_t(expr, block) -> "while (" ^ gen_expr expr ^ ")" ^ gen_block block
